...
## Stage 3 level runtime

- `src/game/gameplay/verticalSlice.ts` управляет первым playable loop: start/tutorial/pass/shot/victory/defeat/pause/retry/next и сохраняет факт завершения обучения в локальном профиле.
- `src/game/levels/schema.ts` описывает универсальный формат уровня: id, version, chapter, localization, episode type, environment, players, teams, routes, decisions, targets, interception zones, cameras, success/failure/star conditions, rewards, difficulty, hints, effects and next level.
- `src/game/levels/validateLevel.ts` выполняет runtime-валидацию обязательных полей, уникальности ID, ссылок на игроков/маршруты/цели, камер, наград, условий успеха/поражения и наличия проходимого пути.
